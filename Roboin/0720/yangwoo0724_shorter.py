...
def set_path(image_shape, thresholded):
    ...
    try:
        if warped[height][:center].min(axis=0) == 255:
            left = 0
        else:
            left = warped[height][:center].argmin(axis=0)
        if warped[height][center:].max(axis=0) ==0:
            right = width
        else:
            right = center+warped[height][center:].argmax(axis=0)
        #print('cccccccccccccccccccccccccccccccc')
        center = int((left+right)/2)
        
        forward =int(first_nonzero(warped[:,center],0,height))-1
        #print('ddddddddddddddddddddddd')
        left_line = first_nonzero(warped[height-forward:height,center:],1, width-center)
        right_line = first_nonzero(np.fliplr(warped[height-forward:height,:center]),1, center)
...
# =============================================================================
            ...
            print(total_time)
            cv2.imshow('analysis',thresholded)
            rawCapture.truncate(0)
